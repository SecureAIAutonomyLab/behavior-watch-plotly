...
class TimeStampCreator {
    ...
    //    DESCRIPTION: When called the getDateOnly method uses Apples services to retrieve the current data. The data is formatted in a specific way and then returned as a string showing the current timestamp. The input paramater is of data type TimeInterval and is used to determine what date the timestamp should be created for by using the TimeInterval to specify how long ago from today to make the timestamp for.
    //    PARAMS: 
    //    RETURNS:
