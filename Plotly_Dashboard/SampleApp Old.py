...
# for data in health_data:
#     data['Timestamp'] = pd.to_datetime(heart_rate_df['Timestamp'], format="%m/%d/%Y %H:%M:%S:%f")
...
def plot_graphs(heart_rate_df, blood_oxygen_df, noise_exp_df, acc_resultant_df, acc_xyz_df, sleep_df):
    ...
    fig2 = px.line(blood_oxygen_df, x='Timestamp', y='Sensor_Info', labels={"Timestamp": "Time","Sensor_Info": "Blood Oxygen(%)"},title="Blood Oxygen Saturation")
    fig3 = px.line(noise_exp_df, x='Timestamp', y='Sensor_Info', labels={"Timestamp": "Time","Sensor_Info": "Audio Exposure(dB)"},title="Environmental Audio Exposure")
