...
health_data = [heart_rate_df, blood_oxygen_df, noise_exp_df, temp_sleep_df]
...
for data in [acc_resultant_df, acc_xyz_df]:
    data.reset_index(inplace=True, drop=True)
for df in [acc_resultant_df, acc_xyz_df]:
    for i in range(len(df.index)):
        time = df.at[i, 'Timestamp']
...
temp = [acc_resultant_df, acc_xyz_df]
...
def update_figure(user_option, checkbox_values):
    ...
    health_data = [heart_rate_df, blood_oxygen_df, noise_exp_df, sleep_df]
